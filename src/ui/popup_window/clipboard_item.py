...
class ClipboardItem(QFrame):
    ...
    def update_item_data(self, new_item_data: Dict):
        """Update widget with new item data (for object pooling)"""
        try:
            # ✅ OPTIMIZATION: Update data without recreating widget
            self.item_data = new_item_data
            self.setProperty("selected", False)
            self.setProperty("pressed", False)

            # Clear and rebuild UI content
    ...
    def clear_item_data(self):
        """Clear item data for object pooling"""
        try:
            # ✅ OPTIMIZATION: Clear data and reset state
            self.item_data = {}
            self.is_hovered = False
            self.setProperty("selected", False)
            self.setProperty("pressed", False)

            # ✅ FIX: Don't clear widgets, just reset data
            # self._clear_content_widgets()  # Skip this to avoid layout issues

        except Exception as e:
            logger.error(f"Error clearing item data: {e}")

    def _clear_content_widgets(self):
        """Clear content widgets from layout - FIXED"""
        try:
            # ✅ FIX: More robust content clearing
            layout = self.layout()
            if not layout or layout.count() < 3:  # Need icon, content, actions
                return

            # Get content layout (middle item between icon and actions)
            ...
            # ✅ FIX: Collect widgets first, then remove safely
            widgets_to_remove = []
            for i in range(content_layout.count()):
                child_item = content_layout.itemAt(i)
                if child_item and child_item.widget():
                    widgets_to_remove.append((child_item, child_item.widget()))

            # Remove widgets safely
            for child_item, widget in widgets_to_remove:
                widget.setParent(None)
                content_layout.removeItem(child_item)
        except Exception as e:
            logger.error(f"Error clearing content widgets: {e}")

    def _rebuild_content(self):
        """Rebuild content widgets with new data"""
        try:
            # Find content layout
            layout = self.layout()
            if layout and layout.count() >= 2:
                ...
                if content_item:
                    ...
                    if content_layout:
                        ...
                        if self.item_data.get("content_type") == "image":
                            preview_widget = self._create_image_preview()
                        else:
                            preview_widget = self._create_text_preview()
